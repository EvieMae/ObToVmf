...
class Main(QtWidgets.QMainWindow):
    ...
    def _hull_spec(entry):
        ...
        if isinstance(entry, dict):
            rot = [float(a) for a in (list(entry.get("rot") or []) + [0, 0, 0])[:3]]
        ...
        return {"type": "box", "bounds": [float(c) for c in entry[:6]],
                "axis": "+x", "top_scale": 0.5, "rot": [0.0, 0.0, 0.0]}

    def _spawn_box(self, vtk_bounds, shape="box", axis="+x", top_scale=0.5, rot=(0, 0, 0)):
        if self.plotter is None:
            return
    ...
    def _respawn_boxes(self):
        ...
        specs = [(e["bounds"], e["type"], e["axis"], e["top_scale"],
                  e.get("rot") or [0, 0, 0]) for e in self.boxes]
        self._clear_boxes()
        for vb, t, ax, ts, rot in specs:
            self._spawn_box(vb, t, ax, ts, rot)
